  ...
  async updateBlockingRules() {
    try {
      ...
      await chrome.declarativeNetRequest.updateDynamicRules({
        removeRuleIds: [1] // Remove the default rule
      });
      ...
      if (rules.length > 0) {
        await chrome.declarativeNetRequest.updateDynamicRules({
          removeRuleIds: [1], // Remove default rule
          addRules: rules
        });
        
        console.log(`🛡️ Updated blocking rules for ${rules.length} domains:`, 
      ...
      switch (message.type) {
        ...
        case 'BLOCK_SITE': // Add alias for backward compatibility
          if (this.currentSession.domain) {
            const blockResult = await this.blockingManager.addBlockedSite(this.currentSession.domain);
            sendResponse(blockResult);
          } else {
            sendResponse({ success: false, error: 'No active site to block' });
          }
          break;
